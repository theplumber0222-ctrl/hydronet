...
export function RegisterForm() {
  ...
  const goldIntent = searchParams.get("plan") === "gold";

  const joinGoldReturn = `/join/gold?billing=${billing}`;
  const loginAfterRegister = goldIntent
    ? `/login?registered=1&callbackUrl=${encodeURIComponent(joinGoldReturn)}`
    : "/login?registered=1";

  const loginForExistingHref = goldIntent
    ? `/login?callbackUrl=${encodeURIComponent(joinGoldReturn)}`
    : "/login";
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(loginAfterRegister);
    } catch {
  ...
  return (
    <main className="mx-auto flex max-w-md flex-1 flex-col justify-center px-4 py-16">
      ...
      <p className="mt-2 text-sm text-slate-400">
        {goldIntent ? t("register.subtitleGold") : t("register.subtitle")}
      </p>
      ...
      <p className="mt-6 text-center text-sm text-slate-400">
        ...
        <Link href={loginForExistingHref} className="link-sky">
          {t("register.signIn")}
        </Link>
      </p>
    </main>
