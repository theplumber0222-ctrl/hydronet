...
import { registerHrefFromJoinGoldCallback } from "@/lib/join-gold-intent";
...
export default async function LoginPage({
  ...
  const registerHref = registerHrefFromJoinGoldCallback(p.callbackUrl);
  ...
  return (
    <main className="mx-auto flex max-w-md flex-1 flex-col justify-center px-4 py-16">
      ...
      <p className="mt-6 text-center text-sm text-slate-400">
        {t(d, "login.noAccount")}{" "}
        <Link href={registerHref} className="link-sky">
          {t(d, "login.register")}
        </Link>
      </p>
